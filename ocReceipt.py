...
def main():
    ...
    for row_index in range(0, img2.shape[1]):
        ...
        for column_index in range(0, img2.shape[0] - 1):
            block_brightnesses = dict([(0, 255), (-1, 255), (1, 255)])
            allowed_height_diffs = [0]
            if actual_row_index - row_index <= 20:
                allowed_height_diffs.append(-1)
                allowed_height_diffs.append(-2)
                allowed_height_diffs.append(-3)
            if row_index - actual_row_index <= 20:
                allowed_height_diffs.append(1)
                allowed_height_diffs.append(2)
                allowed_height_diffs.append(3)
            for height_diff in allowed_height_diffs:
                actual_height = actual_row_index + height_diff
                if actual_height < 0 or actual_height >= img2.shape[1]:
                    block_brightnesses[height_diff] = -1
                    continue
                pixels = []
#                for step in range(5):
#                    pixels.append(img2[column_index*5 + step, actual_height])
                pixels.append(img2[column_index, actual_height])
                block_bright = map(lambda pxl: pxl[2]*0.2126+pxl[1]*0.7152+pxl[0]*0.0722, pixels)
                block_brightnesses[height_diff] = math.ceil(sum(block_bright))
        ...
        if int(avg_row_brightness) >= 254:
            ...
            rows_to_color.append((row_index, [instruction[0] for instruction in row_steps]))
